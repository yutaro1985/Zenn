...
5年ほど前、以下の記事を書きました。
@[card](https://zenn.dev/yutaro1985/articles/daa2aaa6cc9907)
...
- 対象: IAMを日常運用しているが、最新の変更点を追い切れていない人
- ゴール: IAM周辺サービスの進化がベストプラクティスにどう影響したかを整理する
- 得られるもの: フェデレーション/MFA/最小権限の「2512時点の運用の今どき」が分かる
...
ここだけ先に読む: Identity CenterとAccess Analyzerの進化で「長期アクセスキーを減らす」「最小権限を削る運用」が現実的になりました。
...
ざっくり言うと、公式のベストプラクティスはフェデレーション・MFA・最小権限・ガードレールが中心軸です。
...
- フェデレーション: 外部のIDプロバイダー（例: Microsoft Entra ID / Okta / Google Workspace）で認証し、異なる組織やドメイン間で認証情報を安全に共有・連携させる仕組み。AWSにおいてはこれにより短時間の一時認証情報が発行され、サービスを利用できる。
- IAMロール: ユーザーやサービスが「引き受けて使う」権限のまとまり（AssumeRoleで一時認証情報を得る）
- IAM Access Analyzer: リソースの公開/クロスアカウントアクセス、未使用アクセス、ポリシーの安全性などを分析・可視化するサービス
...
[個々の IAM ユーザーを作成する](https://zenn.dev/yutaro1985/articles/daa2aaa6cc9907#個々の-iam-ユーザーを作成する)
[IAM ユーザーへのアクセス許可を割り当てるためにグループを使います](https://zenn.dev/yutaro1985/articles/daa2aaa6cc9907#iam-ユーザーへのアクセス許可を割り当てるためにグループを使います)
...
過去記事の[アクセスキーを共有しない](https://zenn.dev/yutaro1985/articles/daa2aaa6cc9907#アクセスキーを共有しない)がより強固に行いやすくなりました。素晴らしいことですね。
...
[ロールを使用してアクセス許可を委任する](https://zenn.dev/yutaro1985/articles/daa2aaa6cc9907#ロールを使用してアクセス許可を委任する)
...
これは以前からずっとそうですね。
[MFAの有効化](https://zenn.dev/yutaro1985/articles/daa2aaa6cc9907#mfaの有効化)
...
これも考え方自体は以前と同じです。
[最小限の特権を認める](https://zenn.dev/yutaro1985/articles/daa2aaa6cc9907#最小限の特権を認める)
...
まずこのへんから見てみると良いかなと思います。

- ルートユーザーのMFAを必須にし、既存のルートアクセスキーがあれば削除する
- 日常作業でアクセスキーを使っているなら、`aws login`が使えないか検討する
- 使っていないアクセスキー/ユーザー/ロールを棚卸しして削除する
...
https://www.youtube.com/watch?v=kwpjwPijmjU

@[speakerdeck](4c45227d5dbb421cafe2884a6e38f8fa)
