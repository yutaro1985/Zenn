...
それではおもしろくないので、Terraform Registryで広く使われているコミュニティモジュールをラップした独自モジュールを作ってVPCを作るところまでやってみます。

### コミュニティモジュールについて

Terraform Registryには広く使われているVPCモジュールが存在します。
以下では、terraform-aws-modulesコミュニティが公開しているVPCモジュールを使います。
AWS公式のモジュールではありません。
[vpc module](https://registry.terraform.io/modules/terraform-aws-modules/vpc/aws/latest)

:::message alert
なお、これは2023年時点の検証です。後年、terraform-aws-modules内の別のTerraformコミュニティモジュールで、思想的なメッセージへの同意を前提にしたvariableが現在もそのまま残っている件を知ってから、自分はterraform-aws-modules配下のモジュールの利用を避けています。
思想を主張するのは自由ですし、自分はその思想に賛成も反対もしませんが、モジュールを利用することと思想に同意することは異なるため、モジュールの使用と思想への同意を一緒くたにされたくないからです。
:::

VPCは非常に必要なパラメータと関連リソースが多く、自分でゼロから作るのは結構たいへんです。
とはいえ、このモジュールもあらゆるケースに対応すべく非常に多くのパラメータがあるため、自分が使うときはこうした汎用モジュールをラップして使うことが多いです。
（これがよいのか悪いのかはよくわかりません…）
