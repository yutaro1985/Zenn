...
[tasks.dev]
run = "pnpm dev"
env = { _.source = { path = "./scripts/load-dev-secrets.sh", redact = true } }
```
...
scriptをtaskごとに分けておくと、開発用のtaskはdeploy用secretを読まずに済みます。
deploy用なら、たとえば `load-deploy-secrets.sh` の中で `DEPLOY_TOKEN` だけを読む形に分けるイメージです。

ここでは、実在する例としてAWS CLIでSecrets Managerを読む形にしています。
ただし大事なのはコマンド名そのものではなく、外部システムから値を取り込み、必要なtaskの `env` として渡す流れです。
...
[tasks.dev]
run = "pnpm dev"
env = { DATABASE_URL = { required = "各自の DB 接続文字列を設定してください" } }

[tasks.deploy]
run = "pnpm run deploy"
env = { DEPLOY_TOKEN = { required = true, redact = true } }
```

`required` は、「この値がないと実行できない」を設定に書けます。
READMEやWikiに別で書くより、実際に使うtaskへ寄せられます。
...
このあと載せる実運用例では `cargo:watchexec-cli` を `tools` に含めていますが、これはcargo backendを使うので `cargo` を使える状態が前提です。
Node中心のprojectでも、そのためだけにRustを入れるのが許容できるならこの形でまとまります。
...
run = "pnpm dev"
env = { _.source = { path = "./scripts/load-dev-secrets.sh", redact = true }, DATABASE_URL = { required = "ローカル開発で使う接続先を設定してください。mise.local.toml か事前の環境変数で渡します" } }
...
depends = ["check"]
run = "pnpm run deploy"
env = { _.source = { path = "./scripts/load-deploy-secrets.sh", redact = true }, DEPLOY_TOKEN = { required = "deploy に使う token を設定してください。mise.local.toml か事前の環境変数で渡します", redact = true } }
```
...
また、dev用とdeploy用で読むsecretが違うなら、`_.source` のscript自体も分けておくほうが安全です。
