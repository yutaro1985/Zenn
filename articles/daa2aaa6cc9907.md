...
zennになにか書こうと思って書くのも今回が初ですね。

さて、今回は個人的に以前から非常に気になってしまう使われ方が多いIAMの使い方について、改めてネタにしようと思います。
...
とりあえず真っ先に気をつけたほうがいいことや注意してほしいことは
...
AWSを使うにあたっては、まずマネジメントコンソールにアクセスするためのIAMユーザを作成するのが最初です。（後述しますが、**ルートユーザをそのまま使うのはやめましょう**。）

あと、たまにIAMユーザのことをアカウントと呼ぶ人がいますが、AWSにおいてはアカウントという言葉はユーザを指す言葉ではないです。
この辺はサーバーワークスさんのブログの[この記事](https://blog.serverworks.co.jp/tech/2020/02/03/awsaccountforbeginner/)がわかりやすいです。
アカウントに対してIAMユーザを作成してコンソールにアクセスする、みたいなのをイメージしておけばとりあえずは問題ないです。
...
本当に最初の少人数が触る状態とか、そもそも限られた人しか触らないみたいな状態なら一旦はいいかなと思いますが、AWSに触る人が増えてきたら適切に権限を設計する必要があります。
...
- rootユーザで初回ログイン時、以後使用するためのIAMユーザを作成する
	- 基本的に必要がなければ以後rootユーザは使わない。
	- rootユーザにはMFAを設定しておくことを推奨
- IAMユーザの不要なアクセスキーを見直す
	- 棚卸しして使ってなさそうなら無効化する
...
基本≠簡単です。
IAMを使うに当たり真っ先に注意すべきことをここにまとめます。
...
AWSのアカウント作成時にはIAMユーザではなくメールアドレスとパスワードでログインすることになりますが、ルートユーザの使用は最小限にとどめましょう。
なぜなら、ルートユーザは最強の権限を保持しており何でも行えてしまうからです。
IAMユーザでも強い権限を与えることは可能ですが、アカウントの設定メールアドレスやパスワード自体を変えてしまえたり、サポートプランのレベルを変更したりなど、ルートユーザでしかできないことがあります。

この項目ではルートユーザのアクセスキーのロックについて書かれています。
アクセスキーは万が一それが漏れてしまうとアクセスキーの権限を使ってコンソールログインなしにAWSアカウントに対して好きな操作を行えてしまいます。
よくあるのが高いEC2インスタンスを大量に立てて仮想通貨をマイニングするのに使われるとかですね。
その気になればAWS上に構築したものをすべて壊すことももちろん可能ですし、悪意を持って行えば高いサービスを使いまくって利用者を破産に追い込むこともできるでしょう。（万が一そんな事になってしまったら、まず急いでサポートに相談することをおすすめします。）
これはアクセスキーに限らず、ルートユーザのコンソールログイン用のパスワードが漏れても同じことです。
**ルートユーザのアクセスキーは原則必要ないのでもし作成していたら即刻削除しましょう。**
ルートユーザのパスワードは十分に複雑なものにし、できれば後述するMFAも有向にしましょう。
...
↑でルートユーザを原則使わないと書きましたが、それに伴い**AWSアカウントを作ったら真っ先にログイン用のIAMユーザを作成してください。**
自分が使うのであれば権限は一旦Administrator Accessでとりあえずいいと思います。
本来は誰がどのような権限が必要かを見て適切に権限を設定する必要があります（後述）

当然のことながら、パスワードは十分に複雑なものにしましょう。
他のサービスでも同じことですが、簡単に予測できるパスワードや短すぎるパスワードを今でも平然と使ってるケースはよく見ますし、使い回しもたくさん見ます。
aを@にするとかOを0にするとかははるか昔の対策で今ではほとんど無意味と考えていいです。
...
確率は低くても、一回でもパスワード認証を突破されたら情報は漏れるし好き放題されるしで非常にリスクが高いです。
...
MFAに使えるものは色々ありますが、個人的には[Authy](https://authy.com/)をおすすめします。^[ちなみに、多要素認証と二段階認証は紛らわしいですが別の概念です。
...
これを守ると、同じAWSアカウントを使うのに複数のIAMユーザを作ることが必要になりますが、それぞれに個別に権限を振るのではなく、IAMグループを使ってそのグループに所属させることによって権限を設定しましょう。
これは単純に考えても、例えば10人ユーザがいたとしてそれぞれ1人ずつ権限設定をしていくのは面倒です。
役割ごとにIAMグループを作ってそれぞれのグループに必要な権限設定をして、そのグループにユーザを所属させて権限を設定したほうが管理上も楽です。
例えば管理者のグループには強めの権限を振っておいて、開発者は権限を弱めにするなど。

## ユーザーのために強度の高いパスワードポリシーを設定する。

[ユーザーのために強度の高いパスワードポリシーを設定する。](https://docs.aws.amazon.com/ja_jp/IAM/latest/UserGuide/best-practices.html#configure-strong-password-policy)

これはアカウント自体の設定で、各IAMユーザが設定できるパスワードについてルールを設けることができます。
...
アルファベット、数字、記号すべてを使うことを必須にすることもできますが、そこは必須化しなくてもいいかなと個人的には（必須化することによってそれぞれが1文字も含まれないパターンが候補から消えるため）
...
なぜかというと、パスワードを変更するのは多くは人間なので、変更時に安易なパスワードを設定してしまう傾向が強く、結局人間がセキュリティを低下させてしまうからです。
...
AWSの内部からAWSの各サービスを利用する場合は原則としてIAMロールを使用してください。
ごく単純なところだと、例えばEC2をALBを使ってスケーラブルな構成にする際にサーバ上に直接アクセスキーを配置しておくと、サーバの台数分アクセスキーが複製されることになります。
あとはルートユーザのアクセスキーと同様、アクセスキーが漏れるとコンソールにログインしていなくてもそのアクセスキーのユーザの権限でできる任意の操作をされてしまいます。^[アクセスキーをアプリにハードコードしてコミットしてGitHubに上げてしまい不正利用された、みたいな話は調べると結構出てきます。 [
...
これによりユーザ自体に権限を持たせずに、権限が欲しいときだけに一時クレデンシャルを発行してそれを設定することによりAWSの操作を行うことができます。
...
AdministratorAccessを持っているユーザのアクセスキーをバンバン発行してるのを見たりしますけど、危ないのでなるべく避けましょう。
また、キーだけ必要なユーザの場合にはそもそもパスワードを設定する必要がありません。パスワードはコンソールにログインするために必要なものだからですね。

貼ったドキュメントにもありますが、下記あたりを参考に対処するといいでしょう。
...
このあたりはIAMユーザ、グループ、ロール、ポリシーのそれぞれの関係性を理解してないとイメージしづらいかもしれないので、
わかりやすいこちらの記事を是非御覧ください。
...
自分で権限設定を細かく設定するのももちろんいいんですが、AWSのマネージドポリシーがあります。
...
インラインポリシーとは、ざっくり言うとIAMユーザ・IAMロールに個別に設定するポリシーのことで、他のユーザやロールに使い回すことができません。
基本的にはインラインポリシーは使わず、マネージドポリシーを作成してそれを割り当てることをおすすめします。
